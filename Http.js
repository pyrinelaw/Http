 ...
 * @desc    Http请求控件
 * @github  https://www.github.com/pyrinelaw
 */
    ...
    request.onreadystatechange = function() {
        ...
        if(request.readyState == 4) {
            ...
            if(request.status == 200 || request.status == 304){
                ...
                if(request.onData) {
                    if(url.indexOf("user/status")==-1) console.warn("请求地址："+url+" | 耗时："+((endTime-startTime)/1000).toFixed(2)+"秒 | 返回数据长度:"+(request.responseText.length/1024).toFixed(2)+"k");
                    console.warn("数据回来了: ");
